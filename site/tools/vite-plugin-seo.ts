...
const decode = (value: string): string =>
  value.replace(/&(#x[\da-f]+|#\d+|amp|quot|apos|lt|gt|nbsp);/gi, (entity, key: string) => {
    if (key[0] === '#') {
      const code =
...
const attributes = (tag: string): Record<string, string> =>
  Object.fromEntries(
    Array.from(tag.matchAll(/([\w:-]+)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/g), (m) => [
      m[1].toLowerCase(),
      decode(m[2] ?? m[3] ?? m[4]),
    ]),
  );
const tags = (html: string, name: string): Record<string, string>[] =>
  Array.from(html.matchAll(new RegExp(`<${name}\\b(?:[^>"']|"[^"]*"|'[^']*')*>`, 'gi')), (m) =>
    attributes(m[0]),
  );

export function readMetadata(html: string): {
  title: string;
  description?: string;
  robots?: string;
  canonical?: string;
  ...
  const metas = Object.fromEntries(
    tags(html, 'meta')
      .filter((a) => a.name || a.property)
      .map((a) => [(a.name ?? a.property).toLowerCase(), a.content ?? '']),
  );
  ...
  return {
    ...
    robots: metas.robots,
    canonical: tags(html, 'link').find((a) =>
      a.rel?.toLowerCase().split(/\s+/).includes('canonical'),
    )?.href,
    metas,
  };
}
...
function structuredData(
  ...
  if (collection) {
    const entries =
      path === '/'
        ? LANES.map((item) => ({
            url: abs(laneEntry(item)),
            name: item.name,
            description: item.kicker,
          }))
        : CURATED.map((item) => ({
            url: abs(curatedEntry(item)),
            name: item.name,
    ...
    graph.push({
      '@type': 'ItemList',
      '@id': `${url}#collection`,
      name: path === '/' ? 'Worlds and collections' : 'Curated explorations',
    ...
    graph.push({
      ...
      '@id': `${url}#experience`,
      url,
      name: experience?.name ?? lane?.name ?? title,
      description,
      isPartOf: { '@id': experience ? abs('/curated/#collection') : website },
      ...(experience
        ? { citation: experience.sources.map((source) => source.url), image: abs(experience.image) }
        : {}),
    });
  }
  if (path !== '/') {
    ...
    if (experience) crumbs.push({ name: 'Curated', item: abs('/curated/') });
      ...
      crumbs.push({ name: 'Lab', item: abs('/lab/') });
...
export function stampSeo(
  html: string,
  path: string,
  options: SeoOptions,
  image: SocialImage = { path: '/og.png', width: 1200, height: 630, alt: options.siteName },
): string {
  const current = readMetadata(html),
    title = current.title || options.siteName,
    description = current.description || options.description;
  const abs = (value: string): string => new URL(value, `${options.siteUrl}/`).href;
  const canonical = current.canonical || abs(path),
    imageUrl = current.metas['og:image'] || abs(image.path);
  const lines: string[] = [];
  const addMeta = (kind: 'name' | 'property', key: string, value: string): void => {
    if (!(key in current.metas))
      lines.push(`    <meta ${kind}="${key}" content="${esc(value)}" />`);
  };
  ...
  if (!/application\/ld\+json/i.test(html) && !/\bnoindex\b/i.test(current.robots ?? '')) {
    const data = JSON.stringify({
      ...
      '@graph': structuredData(path, title, description, options),
    }).replaceAll('<', '\\u003c');
    ...
    closeBundle() {
      ...
      for (const file of files) {
        const before = readFileSync(file, 'utf8'),
          path = urlPath(relative(outDir, file));
        const after = stampSeo(before, path, options, imageFor(path));
        if (after !== before) writeFileSync(file, after);
        const metadata = readMetadata(after),
          url = new URL(path, `${options.siteUrl}/`).href;
        if (!/\bnoindex\b/i.test(metadata.robots ?? '') && metadata.canonical === url)
          urls.push(url);
      }
