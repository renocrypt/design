...
for (const file of files) {
  const html = readFileSync(file,'utf8'), meta = readMetadata(html), path = urlPath(relative(root,file)), canonical = SITE.origin+path;
  assert.equal(meta.canonical, canonical, `${path}: canonical`);
  assert.ok(meta.description, `${path}: description`);
  assert.ok(meta.metas['og:title'] && meta.metas['og:image'] && meta.metas['twitter:card'], `${path}: share card`);
  const noindex = /\bnoindex\b/i.test(meta.robots ?? '');
  assert.equal(sitemap.includes(`<loc>${canonical}</loc>`), !noindex, `${path}: sitemap eligibility`);
  if (!noindex) {
  ...
  for (const match of html.matchAll(/(?:href|src)="([^"#]+)"/g)) {
    ...
    const url = new URL(value,canonical), target = resolve(root, '.'+decodeURIComponent(url.pathname));
    assert.ok(target.startsWith(root+'/') || target===root,`${path}: asset stays inside the site`);
...
for (const path of [...LANES.map(laneEntry),...CURATED.map(curatedEntry)]) {
  ...
  assert.ok(existsSync(join(root,path,'index.html')),`registered destination ${path} is published`);
}
...
for (const entry of CURATED) assert.ok(gallery.includes(`href="${curatedEntry(entry)}"`),'the gallery links to its experience without client scripting');
assert.ok(readFileSync(join(root,'llms.txt'),'utf8').includes('/curated/bikini-atoll/'),'the authored discovery guide includes the new route');
assert.doesNotMatch(sitemap,/<lastmod>/,'do not publish build dates as content changes');
console.log(`Verified ${files.length} HTML pages: routes, assets, canonical URLs, social metadata, structured data, sitemap, and static navigation.`);
