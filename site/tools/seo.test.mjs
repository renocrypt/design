...
const document = (head) =>
  `<!doctype html><html lang="en"><head>${head}</head><body><h1>A real page</h1></body></html>`;

test('standalone HTML retains multiline descriptions, reordered attributes, and entities', () => {
  const html = document(
    `<title>Coral &amp; the sea</title><meta content='A &quot;quiet&quot; place &amp; a living ocean'\n data-author='yes' NAME='description'>`,
  );
...
test('partial authored social cards receive missing tags without overwriting their title', () => {
  const html = document(
    `<title>Bikini Atoll</title><meta property='og:title' content='A title chosen by the author'>`,
  );
  const built = stampSeo(html, '/curated/bikini-atoll/', options);
  const meta = readMetadata(built);
  ...
  assert.equal(
    stampSeo(built, '/curated/bikini-atoll/', options),
    built,
    'a second build pass is idempotent',
  );
});

test('an authored social image is reused without invented dimensions', () => {
  const built = stampSeo(
    document(
      `<title>Own image</title><meta property="og:image" content="https://example.com/own.jpg">`,
    ),
    '/own/',
    options,
  );
...
test('curated structured data describes the work and actual navigation parents', () => {
  const built = stampSeo(
    document('<title>Bikini Atoll</title>'),
    '/curated/bikini-atoll/',
    options,
  );
  const data = JSON.parse(
    built.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/)[1],
  );
  const crumbs = data['@graph'].find((node) => node['@type'] === 'BreadcrumbList');
  assert.deepEqual(
    crumbs.itemListElement.map((item) => item.item),
    [SITE.origin + '/', SITE.origin + '/curated/', SITE.origin + '/curated/bikini-atoll/'],
  );
  assert.equal(
    data['@graph'].find((node) => node['@type'] === 'CreativeWork').name,
    'Bikini Atoll',
  );
});
...
test('structured text cannot terminate its script element', () => {
  const built = stampSeo(
    document('<title>&lt;/script&gt;&lt;script&gt;unexpected&lt;/script&gt;</title>'),
    '/safe/',
    options,
  );
...
test('noindex pages preserve their indexing decision and do not gain collection data', () => {
  const built = stampSeo(
    document('<title>Not here</title><meta content="noindex, follow" name="robots">'),
    '/404.html',
    options,
  );
