...
**Correction, same day**: while fixing the missing icon I also redesigned the mark (cream ground → ink ground with the three lane bars) and justified it with a 16px legibility claim I had never actually measured — I only proof-sheeted the new one. The user's mark was correct and the redesign was out of scope for a bug fix. Reverted: the original cream / pale-ink-red bars are back in both `favicon.svg` and `make-favicons.mjs`, and the ICO was re-checked at 16px native and 8× nearest (it reads fine).

Standing lesson: a fix for *missing* is not licence to change *design*, and a legibility claim without a render behind it is exactly the kind of assertion this project bans. Geometry is duplicated in `favicon.svg` and `make-favicons.mjs`; both files say so.

## Hub night mode + lane bookkeeping — 2026-07-30

Two findings, both surfaced by the user asking why world 00's card is sized differently and why it hurts at night.

**Night contrast was broken on every filled surface, not just mint.** `--ink` flips to cream `#f4e9e1` at night, but the toy-box fills and the two cream cards deliberately do not flip — and `.gate`, `.room-card`, `.bench-card`, `.man-card`, `.rule-ico`, `.foot`, `.foot-logo` and `.pill-link` all painted their text with `--ink`. Measured before: mint card **1.29:1**, amber utility card 1.51:1, and the rules card and footer at **1.00:1** — cream text on a cream ground, invisible. The hard-coded dark values already in the file (card hairlines `rgb(14 13 11 / 30%)`, `.rules p`, `.pill-link--static`) prove the original intent; only the tokened text was left following the flip. Fix: `--ink-on-hue`, black in both themes, for anything painted on a fill; `--ink` now means "on paper" only.

**Mint also glared, which is a separate problem from its text.** Mint sat at L 0.634 and amber at L 0.531 against a `#171310` page, ~3× above the cobalt/violet/red band (0.18–0.27), and mint carries the widest card on the page. Both darkened at identical hue and saturation (L 64%→34%, 50%→34%) to land inside that band: mint `#189679`, amber-fill `#ad7900`. Amber needed a separate `--amber-fill` token because `--amber` is also the ticker's text colour on the red band, where darkening would have inverted the problem — so small accents (ticker, the 34px pixel blocks) keep the undimmed hue. Area is what glares, not saturation. Verified in-page at night: mint card text 5.25:1 (from 1.29), fill-vs-page 4.99:1 (from 12.02); rules card and footer 16.27:1 (from 1.00); mint is now the best-contrasting lane fill at night rather than the worst.

**00 is a nested hub, and the docs never said so.** `site/public/lab/index.html` is an entrance with its own full identity (`#0c0c0e` ground, bone text, phosphor `#62e6c8`, Clash Display + Newsreader italic + JetBrains Mono, "Pick a *direction*"), opening into S1/S2/S4/S5 — so six designed surfaces here, not five, and three levels deep. It is the one surface with no reference pole because it predates the mapping (built ~2026-07-19 off `award-study-2026-07.md`). The hub's `--mint` is a quote of that page's accent, which is why the night dimming is hub-only: on the lab's dark ground the undimmed green is a small accent with nothing to glare against. Both files now say so.

**Two lane lists were still hand-written.** The marquee band hard-coded `01 Noir · 02 Chrome · 03 Monument · 04 Pulse` twice in `index.html`, and the closing CTA hard-coded "First door — 01 Noir" plus its href — so a new lane would have joined the rail, menu and grid while silently missing both, and § Adding a lane never mentioned them. Now generated by `renderTicker`/`renderFirstDoor` from the registry, filtered to `kind: 'world'` so the archive's absence from a marquee of destinations is a stated rule instead of an oversight. Day output verified byte-identical to the old markup (2 halves, 8 glyphs, same text, same href). Registry header and the checklist now say six consumers, two of them filtered.

**Ordering was contradictory but defensibly so.** 00 is first in the rail and last in the room grid. Kept, with the reason recorded at both sites: the rail is an index (numeral order, 00 as a compact marker above the doors), the grid is a pitch (worlds lead, non-world lanes close as a wide shelf). The differing card size is load-bearing and stays — it says "different kind of thing", which is true, since 00 opens an entrance rather than a world.
