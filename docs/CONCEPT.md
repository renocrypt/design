...
Analytics: one Vite plugin (`site/tools/vite-plugin-google-tag.ts`) stamps the Google tag into every built page at `closeBundle`, including the verbatim-copied archive — a template-level tag would silently miss `public/`.
